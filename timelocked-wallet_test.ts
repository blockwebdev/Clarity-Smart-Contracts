...
/*
Four tests that cover the different cases of lock.
1) Allows the contract owner to lock an amount
2) Does not allow anyone else to lock an amount
3) Cannot be locked more than once
4) Cannot set the unlock height to a value less than the current block height
*/
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        block.receipts[0].events.expectSTXTransferEvent(
            amount, deployer.address, `${deployer.address}.timelocked-wallet`
        );
    },
});
...
/* Testing bestow, a simple function that allows the beneficiary to transfer
the right to claim. Only the beneficiary can successfully call bestow */
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        block.receipts[0].result.expectOk().expectBool(true);
        block.receipts[0].event.expectSTXTransferEvent(
            amount,
            `${deployer.address}.timelocked-wallet`,
            beneficiary.address
        );
